...
def generate_text_and_audio(index, df, words_per_batch):
    ...
    for i, row in enumerate(selected_data.itertuples(), start=1):
        text_lines.append(
            f"{i + start}. {row.Word}. The part of speech is {convert_pos(row.POS)} and the stress falls on the {row.Stress}. Or we can say that the stress is on the {row.Stress}imate syllable."
        )
...
with tabs[2]:
    ...
    if st.session_state["started"]:
        ...
        if st.session_state["completed"]:
            ...
            formatted_texts = [
                f"{i+1}. {row['Word']}. The part of speech is {pos_mapping.get(row['POS'], row['POS'])}, and the stress falls on the {row['Stress']}. Or we can say that the stress is on the {row['Stress']}imate syllable."
